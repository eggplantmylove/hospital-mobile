<template>
  <div>
    ...
    <h4 style="margin: 20px 10px 10px">门诊费用信息</h4>
    <van-collapse v-model="activeNames">
      <van-collapse-item
        v-for="(item, index) in outpatientRecord.pay_list"
        :key="index"
      >
        <template #title>
    ...
    <!-- <van-cell title="合计金额：">
      <p style="color: red">&yen;{{pendingPay.total}}</p>
    </van-cell> -->
...
<script>
import { pendingPay, outpatientPay, outpatientRecord,patList } from "../../api/api";
  ...
  methods: {
    getPendingPay: function () {
      ...
      pendingPay(params).then((res) => {
        this.outpatientRecord = res.data.data;
        this.show = false;
      });
    },
    getPatList() {
