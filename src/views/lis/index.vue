<template>
  <div>
    <van-nav-bar left-arrow @click-left="$router.go(-1)">
      <h4 slot="title" style="color: #fff">检验记录</h4>
    </van-nav-bar>
    ...
    <div style="margin-top: 10px">
      <div class="box">
        ...
        >
          ...
          <div class="user-info">
            <span>姓名：{{item.pat_name}}</span>
           <span>检验日期：{{item.inspect_time}}</span>
          </div>
          <div class="user-info">
            <span>状态：
              ...
              <span>报告日期：{{item.report_time}}</span> 
          </div>
...
<script>
import { getLisList,patList } from "../../api/api";
...
export default {
  ...
  methods: {
    queryLisList() {
      ...
      getLisList(this.params).then((res) => {
    ...
    go_detail(item) {
      ...
      this.$router.push({ path: `lis-detail/`+item.id });
    },
  },
  mounted() {
    ...
    if(this.patObjectStore.his_pat_id!=''){
      this.queryLisList();
    }else{
      this.showPicker = true;  
    } 
